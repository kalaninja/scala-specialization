...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd.aggregate(0)((acc, item) => if (item.mentionsLanguage(lang)) acc + 1 else acc, _ + _)
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    rdd.flatMap(article => langs.filter(lang => article.mentionsLanguage(lang)).map(lang => (lang, 1)))
      .reduceByKey(_ + _)
      .sortBy(-_._2)
      .collect()
      .toList
