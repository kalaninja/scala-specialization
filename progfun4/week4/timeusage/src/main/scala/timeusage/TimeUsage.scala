...
object TimeUsage {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val result = columnNames.foldLeft((ListBuffer[Column](), ListBuffer[Column](), ListBuffer[Column]()))((acc, name) => {
      if (primary.exists(name.startsWith))
        acc._1 += new Column(name)
      else if (work.exists(name.startsWith))
        acc._2 += new Column(name)
      else if (other.exists(name.startsWith))
        acc._3 += new Column(name)

      acc
    })

    (result._1.toList, result._2.toList, result._3.toList)
  }
