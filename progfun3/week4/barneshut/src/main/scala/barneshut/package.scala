...
package object barneshut {
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body]) extends Quad {
    val (mass, massX, massY) = (
      ...
      bodies.foldLeft(0f)(_ + _.x) / bodies.size: Float,
      bodies.foldLeft(0f)(_ + _.y) / bodies.size: Float
    )
    val total: Int = bodies.size

    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        val (w, e, n, s) = (centerX - size / 4, centerX + size / 4, centerY - size / 4, centerY + size / 4)
        val fork = Fork(Empty(w, n, size / 2), Empty(e, n, size / 2), Empty(w, s, size / 2), Empty(e, s, size / 2))
  ...
  class Body(val mass: Float, val x: Float, val y: Float, val xspeed: Float, val yspeed: Float) {
    ...
    def updated(quad: Quad): Body = {
      ...
      def traverse(quad: Quad): Unit = (quad: Quad) match {
        ...
        case Fork(nw, ne, sw, se) => if (quad.size / distance(quad.centerX, quad.centerY, x, y) < theta) {
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val sector_x = (b.x - boundaries.minX) / (boundaries.width / sectorPrecision)
      val sector_y = (b.y - boundaries.minY) / (boundaries.height / sectorPrecision)

      val x = math.min(math.max(sector_x.toInt, 0), sectorPrecision - 1)
      val y = math.min(math.max(sector_y.toInt, 0), sectorPrecision - 1)
