...
object ParallelParenthesesBalancing {
  ...
  def balance(chars: Array[Char]): Boolean = {
    def balanceAcc(chars: Array[Char], opened: Int): Boolean = chars match {
      case Array() => opened == 0
      case Array(_, _*) => chars.head match {
        case '(' => balanceAcc(chars.tail, opened + 1)
        case ')' => if (opened > 0) balanceAcc(chars.tail, opened - 1) else false
        case _ => balanceAcc(chars.tail, opened)
      }
    }
