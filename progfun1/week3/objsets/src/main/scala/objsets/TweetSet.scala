...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    def aggregatedAcc = if (p(elem)) acc.incl(elem) else acc

    right.filterAcc(p, left.filterAcc(p, aggregatedAcc))
  }
...
object GoogleVsApple {
  ...
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(x => google.exists(y => x.text.contains(y)))
