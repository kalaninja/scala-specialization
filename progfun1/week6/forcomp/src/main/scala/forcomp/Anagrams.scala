...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w toLowerCase() groupBy identity map { case (k, v) => (k, v.length) }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case (ch, times) :: xs =>
      for {
        ...
        n <- 0 to times
      } yield if (n == 0) comb else (ch, n) :: comb
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val yMap = y.toMap
    x map { case (ch, times) => (ch, times - yMap.getOrElse(ch, 0)) }
  }.filter(_._2 > 0)
    ...
    def helper(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) {
        ...
        for {
          ...
          s <- helper(subtract(occurrences, i))
        } yield j :: s
      }
    }
